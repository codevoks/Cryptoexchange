...
async function handleRedisSubscription(symbol: string) {
  ...
  await redisSubscribe(ORDERBOOK_CHANNEL, (message: string) => {
    ...
    for (const ws of conns) {
      if (ws.readyState === WebSocket.OPEN) {
        // console.log(
        //   "SENDING ORDER DATA VIA WS on symbol = ",
        //   symbol,
        //   " parsedMessage = ",
        //   parsedMessage
        // );
  ...
  console.log("TRADE WEBSOCKET subsribing to " + TRADE_CHANNEL);
  await redisSubscribe(TRADE_CHANNEL, (message: string) => {
    // console.log("Subscribed to " + TRADE_CHANNEL);
    const conns = clients.get(symbol);
    // console.log("conns is-> " + conns);
    if (!conns) return;
    ...
    for (const ws of conns) {
      if (ws.readyState === WebSocket.OPEN) {
        // console.log(
        //   "SENDING TRADE DATA VIA WS on symbol = ",
        //   symbol,
        //   " parsedMessage = ",
        //   parsedMessage
        // );
